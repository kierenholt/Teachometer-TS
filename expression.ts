...
function toExpressionTree(s, i, commaIsTerminator?):IExpression {
        ...
        && (i+2 >= s.length || s[i+1] != "/" || s[i+2] != "/"))
    {
        if (s[i] == '(') {
            ...
            if (buffer.length == 0) { throw new Error("Range expression missing something before ..");}
            children.push(buffer);
            return new RangeExpression(new SimpleExpression(children,i), s, i);
        }
        else if (s[i] == ",") {
            if (buffer.length == 0) { throw new Error("List expression missing something before ,");}
            children.push(buffer);
